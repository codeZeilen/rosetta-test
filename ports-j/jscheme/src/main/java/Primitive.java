...
public class Primitive extends Procedure {
    ...
    public Object apply(Scheme interp, Object args) {
        ...
        switch (idNumber) {
            ...
            case ODDQ:
                return truth(Math.abs(num(x).doubleValue()) % 2 != 0);
            case EVENQ:
                return truth(Math.abs(num(x).doubleValue()) % 2 == 0);
            case ZEROQ:
                return truth(num(x).doubleValue() == 0);
            case POSITIVEQ:
                return truth(num(x).doubleValue() > 0);
            case NEGATIVEQ:
                return truth(num(x).doubleValue() < 0);
            ...
            case PLUS:
                return numCompute(args, '+', num(0));
            case MINUS:
                return numCompute(rest(args), '-', num(x));
            case TIMES:
                return numCompute(args, '*', num(1));
            case DIVIDE:
                return numCompute(rest(args), '/', num(x));
            case QUOTIENT:
                double d;
                if(x instanceof BigInteger && y instanceof BigInteger) {
                    d = num(x).intValue() / num(y).intValue();
                } else {
                    d = num(x).doubleValue() / num(y).doubleValue();
                }
            ...
            case ABS:
                if(x instanceof BigInteger) {
                    return num(Math.abs(num(x).intValue()));
                } else {
                    return num(Math.abs(num(x).doubleValue()));
                }
            case FLOOR:
                if(x instanceof BigInteger) {
                    return num(Math.floor(num(x).intValue()));
                } else {
                    return num(Math.floor(num(x).doubleValue()));
                }
            case CEILING:
                if(x instanceof BigInteger) {
                    return num(Math.ceil(num(x).intValue()));
                } else {
                    return num(Math.ceil(num(x).doubleValue()));
                }
            case TRUNCATE:
                if(x instanceof BigInteger) {
                    return num(x);
            ...
            case ROUND:
                if(x instanceof BigInteger) {
                    return num(Math.round(num(x).intValue()));
                } else {
                    return num(Math.round(num(x).doubleValue()));
                }
            case EXP:
                if(x instanceof BigInteger) {
                    return num(Math.exp(num(x).intValue()));
                } else {
                    return num(Math.exp(num(x).doubleValue()));
                }
            case LOG:
                if(x instanceof BigInteger) {
                    return num(Math.log(num(x).intValue()));
                } else {
                    return num(Math.log(num(x).doubleValue()));
                }
            case SIN:
                if(x instanceof BigInteger) {
                    return num(Math.sin(num(x).intValue()));
                } else {
                    return num(Math.sin(num(x).doubleValue()));
                }
            case COS:
                if(x instanceof BigInteger) {
                    return num(Math.cos(num(x).intValue()));
                } else {
                    return num(Math.cos(num(x).doubleValue()));
                }
            case TAN:
                if(x instanceof BigInteger) {
                    return num(Math.tan(num(x).intValue()));
                } else {
                    return num(Math.tan(num(x).doubleValue()));
                }
            case ASIN:
                if(x instanceof BigInteger) {
                    return num(Math.asin(num(x).intValue()));
                } else {
                    return num(Math.asin(num(x).doubleValue()));
                }
            case ACOS:
                if(x instanceof BigInteger) {
                    return num(Math.acos(num(x).intValue()));
                } else {
                    return num(Math.acos(num(x).doubleValue()));
                }
            case ATAN:
                if(x instanceof BigInteger) {
                    return num(Math.atan(num(x).intValue()));
                } else {
                    return num(Math.atan(num(x).doubleValue()));
                }
            case SQRT:
                if(x instanceof BigInteger) {
                    return num(Math.sqrt(num(x).intValue()));
                } else {
                    return num(Math.sqrt(num(x).doubleValue()));
                }
            case EXPT:
                return num(Math.pow(num(x).doubleValue(), num(y).doubleValue()));
            case NUMBERTOSTRING:
            ...
            case CHARTOINTEGER:
                return Double.valueOf((double) chr(x));
            case INTEGERTOCHAR:
            ...
            case NEW:
                try {
                    return JavaMethod.toClass(x).getDeclaredConstructor().newInstance();
    ...
    public static Object numCompare(Object args, char op) {
        while (rest(args) instanceof Pair) {
            Double x = num(first(args)).doubleValue(); // TODO: This is inexact
            args = rest(args);
            Double y = num(first(args)).doubleValue(); // TODO: This is inexact
            switch (op) {
                case '>':
                    if (!(x > y)) return FALSE;
                    break;
    ...
    public static Object numCompute(Object args, char op, Number result) {
        if (args == null) {
            switch (op) {
                case '-':
                    if(result instanceof BigInteger) {
                        return num(BigInteger.valueOf(0).subtract((BigInteger) result));
                    } else {
                        return num(0 - result.doubleValue());
                    }
                case '/':
                    if(result instanceof BigInteger) {
                        return num(BigInteger.valueOf(1).divide((BigInteger) result));
                    } else {
                        return num(1 / result.doubleValue());
                    }
                default:
                    return num(result);
            }
            ...
            while (args instanceof Pair) {
                Number x = num(first(args));
                args = rest(args);
                switch (op) {
                    case 'X':
                        if (x.doubleValue() > result.doubleValue()) result = x; // TODO: This is inexact
                        break;
                    case 'N':
                        if (x.doubleValue() < result.doubleValue()) result = x; // TODO: This is inexact
                        break;
                    case '+':
                        if(x instanceof BigInteger && result instanceof BigInteger) {
                            result = ((BigInteger) result).add((BigInteger) x);
                        } else {
                            result = result.doubleValue() + x.doubleValue();
                        }
                        break;
                    case '-':
                        if(x instanceof BigInteger && result instanceof BigInteger) {
                            result = ((BigInteger) result).subtract((BigInteger) x);
                        } else {
                            result = result.doubleValue() - x.doubleValue();
                        }
                        break;
                    case '*':
                        if(x instanceof BigInteger && result instanceof BigInteger) {
                            result = ((BigInteger) result).multiply((BigInteger) x);
                        } else {
                            result = result.doubleValue() * x.doubleValue();
                        }
                        break;
                    case '/':
                        if(x instanceof BigInteger && result instanceof BigInteger) {
                            result = ((BigInteger) result).divide((BigInteger) x);
                        } else {
                            result = result.doubleValue() / x.doubleValue();
                        }
                        break;
