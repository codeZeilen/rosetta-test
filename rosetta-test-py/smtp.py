...
smtp_suite = rosetta.suite("rosetta-test-suites/smtp.rosetta")
...
def smtp_secure_connect_with_timeout(env, host, port, cafile, timeout=None):
    ...
    context.check_hostname = False
    context.verify_mode = ssl.CERT_NONE
    context.load_verify_locations(rosetta.fixture_path(cafile))
    if timeout:
        try:
...
def smtp_starttls(env, smtp, certfile=None, keyfile=None):
    if certfile and keyfile:
        return smtp.starttls(keyfile=rosetta.fixture_path(keyfile), certfile=rosetta.fixture_path(certfile))
    else:
        try:
...
smtp_suite.run(
        ...
        exclude=(
            ...
            # There is no check whether the server suprosetta 8BITMIME
