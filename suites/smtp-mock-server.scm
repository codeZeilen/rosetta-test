(begin    
    ...
    (define (server-message-data server) (list-ref server 11))
    ...
    (define (run-mock-smtp-server server)
        (begin
            ...
            (define (write-prepared-response-code-else alternative-branch) 
                (if (server-has-response-code? server) 
                    (socket-write connection (string-append (server-next-response-code server) "\r\n")) 
                    (alternative-branch)))
            ...
            (define (data-command args)
                ...
                (receive-all-data)
                (write-prepared-response-code-else (lambda () 
                    (socket-write connection "250 2.0.0 OK\r\n"))))
